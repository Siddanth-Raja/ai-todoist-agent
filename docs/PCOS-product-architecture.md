...
**Decision approved September 14, 2026. The external-Blinn-blocker exception below is approved. `7ab17579c26df71591618487282c6050624d32ef` was the prior published baseline before this exception; Git and the Obsidian export manifest determine publication state.** These are approved product requirements, not claims of implemented or shipped capabilities. The College bridge, capture lifecycle and cross-conversation continuity remain unimplemented. See [current engineering state](PCOS-handoff.md).
...
The [handoff's section 73](PCOS-handoff.md#73-approved-product-architecture-and-roadmap-reconciliation) records exact issue ownership and dependencies. With Blinn administrator approval externally pending, SID-249 runs as a parallel paused provider track; it does not block SID-259, SID-250, or the first College continuity slice. SID-259 contract planning and the College continuity foundation may proceed using published TAMU read-only evidence, Personal Gmail where relevant, user-confirmed conversational evidence, and existing Calendar/task architecture, subject to their other prerequisites. The remaining order is SID-259 contract → SID-151 lifecycle → SID-250 domain state → SID-260 reads → SID-261 capture → SID-147 brief → SID-251 surfaces → SID-156 design → SID-157 runtime → SID-262 staging → SID-252 acceptance. Blinn coverage must stay explicitly unavailable/pending and qualify confidence and completeness, never appear healthy, empty, complete, or silently disappear. Full SID-252 acceptance still requires verified Blinn access or a separately approved partial-coverage exception; these dependency changes are not that exception. The SID-148 full-milestone branch remains mandatory. SID-149 is deferred; SID-236 retains SID-252, SID-258 and SID-248 blockers. No implementation is started by this documentation change.
