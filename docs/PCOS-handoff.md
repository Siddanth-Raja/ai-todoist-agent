...
| --- | --- | --- |
| [SID-249](https://linear.app/siddanths-workspace/issue/SID-249/connect-blinn-email-read-only) | Existing Blinn read-only scope unchanged; administrator approval externally pending, with no consent or live verification. | None |
| [SID-259](https://linear.app/siddanths-workspace/issue/SID-259/approve-the-provider-neutral-college-contract-and-ownership-rules) | Planning/contract approval only: identity, claims, authority, capture, correction, lifecycle, closure and explicit pending-Blinn coverage inputs. | None |
...
| [SID-250](https://linear.app/siddanths-workspace/issue/SID-250/build-canonical-college-claims-state-and-attention) | Durable College entities/claims, revisions, atomic event/receipt state and shared attention; retain three-account identity and explicit pending-Blinn coverage using available evidence; no MCP/UI. | SID-259, SID-151, SID-145 |
...
The former direct SID-249 blockers of SID-259 and SID-250 no longer gate contract planning or the first College continuity slice. SID-249 is a parallel externally paused provider track, still In Progress, uncommitted and unverified pending administrator approval; its implementation scope is unchanged. At this published checkpoint, SID-259 remained To Do with no start date; section 74 supersedes that status snapshot. SID-259 and the College continuity foundation may use published TAMU read-only evidence, Personal Gmail where relevant, user-confirmed conversational evidence, and existing Calendar/task architecture, subject to their remaining prerequisites. Blinn stays an explicit unavailable/pending source that qualifies confidence and completeness, never healthy, empty, complete, or silently omitted. SID-150 remains the protected-action/context prerequisite. SID-145/249 preserve three-account source identities and eventual full-acceptance requirements; existing SID-127/146 and SID-131 prerequisites remain. SID-149's other downstream blockers remain unchanged.
...
The preceding readback records the original publication, before the external-pause exception. At that checkpoint, SID-259 was the only newly eligible To Do issue and its work had not started; section 74 supersedes that status snapshot. SID-250 remains blocked by SID-259 and SID-151 (SID-145 is already Done), not by SID-249. Full SID-252 acceptance still requires verified Blinn access or a separately approved explicit partial-coverage exception. Removing these direct blockers is not such an exception and does not satisfy any College implementation or acceptance gate.
...
# 74. SID-259 approved College contract and documentation publication

Siddanth explicitly approved the revised `college-contract/1.0` schema and ownership rules in [PCOS-college-contract.md](PCOS-college-contract.md) and authorized documentation-only publication. The approved contract includes identity and reviewed course binding, evidence/field authority, independent state, event/receipt and concurrency rules, multidimensional coverage, conversational extraction, raw retention versus structured attestation, correction/forgetting, transient context, shared attention, and the bounded assessment-production lifecycle. Specification walkthroughs do not establish runtime acceptance.

Publication is limited to `docs/PCOS-college-contract.md` and `docs/PCOS-handoff.md`: inspect/check the exact documentation diff, stage only those paths, commit and push by normal fast-forward, verify local HEAD, `origin/main`, and GitHub main agree, then export exact committed handoff bytes to Obsidian with a source-path/commit/SHA-256/export-time/state manifest. Git and that manifest identify the resulting publication; SID-259 records the exact commit and verification evidence and may move from In Progress to Done only after these gates pass.
